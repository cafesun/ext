...
class pool: protected simple_segregated_storage < typename UserAllocator::size_type >
{
  ...
  public:
    ...
    :
		...
		next_size(nnext_size), start_size(nnext_size),
		max_size(nmax_size), total_size(0)
    ...
    bool release_memory();
	bool release_memory(size_type &size);
	...
	// Get total size of the default pool. qqbao
	...
	// updates total size ; qqbao
	  total_size += (get_next_size() * get_requested_size());
      return malloc_need_resize();
    }
	
	
	//add qqbao
	void * malloc BOOST_PREVENT_MACRO_SUBSTITUTION(size_type &size)
	{
		// Look for a non-empty storage
		if (!store().empty())
			return (store().malloc)();
		...
		// updates total size ; qqbao
		total_size += (get_next_size() * get_requested_size());
		size += (get_next_size() * get_requested_size());
...
template <typename UserAllocator>
bool pool<UserAllocator>::release_memory(size_type &size)
{
	...
	while (ptr.valid())
	{
		...
		else
		{
			...
			// And release memory
			total_size -= ptr.total_size(); // first updates total size ; qqbao
			size -= ptr.total_size();
